...
class TableDripVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
       if segue.identifier == "detailSegue" {
           if let indexPath = tableView.indexPathForSelectedRow {
               let destination = segue.destination as? DetailDripVC
               if searchController.isActive {
                   destination?.toDetailDripVC = dripMemosFiltered[indexPath.row].dripId
               }else{
                   destination?.toDetailDripVC = dripMemos[indexPath.row].dripId
               }
            }
       }
    }
    ...
    func deleteRows() {
        for indexPathList in sortedIndexPaths {
            //②CoreDataから削除
            managedObjectContext.delete(dripMemos[indexPathList.row])
            do {
            ...
            //③配列から削除
            self.dripMemos.remove(at: indexPathList.row)
        }
        //④tableViewから削除
    ...
    func deleteRow(){
        if swipedIndexPath != nil {
            //②CoreDataからの削除
            managedObjectContext.delete(dripMemos[swipedIndexPath!.row])
            do {
              try managedObjectContext.save()
            } catch {
              let nsError = error as NSError
            ...
            //③配列からの削除
            dripMemos.remove(at: swipedIndexPath!.row)
            //④tableViewからの削除
...
extension TableDripVC: UISearchResultsUpdating {
