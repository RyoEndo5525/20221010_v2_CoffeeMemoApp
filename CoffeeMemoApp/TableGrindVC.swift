...
class TableGrindVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
       if segue.identifier == "detailSegue" {
           if let indexPath = tableView.indexPathForSelectedRow {
               let destination = segue.destination as? DetailGrindVC
               if searchController.isActive {
                   destination?.toDetailGrindVC = grindMemosFiltered[indexPath.row].grindId
               }else{
                   destination?.toDetailGrindVC = grindMemos[indexPath.row].grindId
               }
            }
       }
    }
    ...
    func deleteRows() {
        for indexPathList in sortedIndexPaths {
            //①画像データのエンティティーを削除
            grindImages = getGrindImageId(moc: managedObjectContext, grindId: grindMemos[indexPathList.row].grindId!)
            managedObjectContext.delete(grindImages[0])
            //②CoreDataから削除
            managedObjectContext.delete(grindMemos[indexPathList.row])
            do {
            ...
            //③配列から削除
            self.grindMemos.remove(at: indexPathList.row)
        }
        //④tableViewから削除
    ...
    func deleteRow(){
        if swipedIndexPath != nil {
            //①画像データのエンティティーを削除
            grindImages = getGrindImageId(moc: managedObjectContext, grindId: grindMemos[swipedIndexPath!.row].grindId!)
            managedObjectContext.delete(grindImages[0])
            //②CoreDataからの削除
            managedObjectContext.delete(grindMemos[swipedIndexPath!.row])
            do {
              try managedObjectContext.save()
            } catch {
              let nsError = error as NSError
            ...
            //③配列からの削除
            grindMemos.remove(at: swipedIndexPath!.row)
            //④tableViewからの削除
...
extension TableGrindVC: UISearchResultsUpdating {
